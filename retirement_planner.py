      ...
      <div id="tbl-wrap">
        <div class="tw">
          <table><thead><tr>
            ...
            <th>Tax Bracket</th><th>IRMAA</th><th>End Balance</th>
...
// Note: simplified for offline modeling; can be refined later.
const STATE_BRACKETS_CA = {
  Single: [{r:0.01,to:10000},{r:0.02,to:25000},{r:0.04,to:50000},{r:0.06,to:100000},{r:0.08,to:250000},{r:0.093,to:350000},{r:0.103,to:500000},{r:0.113,to:1000000},{r:0.123,to:1e9}],
  MFJ:    [{r:0.01,to:20000},{r:0.02,to:50000},{r:0.04,to:100000},{r:0.06,to:200000},{r:0.08,to:500000},{r:0.093,to:700000},{r:0.103,to:1000000},{r:0.113,to:2000000},{r:0.123,to:1e9}]
};
const STD_DED_CA = {MFJ:9808, Single:4904};
...
/** Compute a simple CA state tax (standard deduction only) */
function computeStateTaxCA(agi, filing, multiplier=1){
  const std = Math.round((STD_DED_CA[filing]||STD_DED_CA.MFJ) * multiplier);
  const bs = (STATE_BRACKETS_CA[filing]||STATE_BRACKETS_CA.MFJ).map(b=>({r:b.r,to:Math.round(b.to*multiplier)}));
  // CA does not tax Social Security — caller should pass AGI excluding SS if needed
  const taxable = Math.max(0, agi - std);
  let tax=0, prev=0;
  for(const b of bs){
...
function evalConversionOptions(provAGI, tradBal, filing, multiplier=1){
  ...
  for(const t of targets){
    ...
    const newState = computeStateTaxCA(Math.max(0, provAGI+convAmt), filing, multiplier);
...
function renderTable(accumRows, retireRows, inp){
  ...
  if(accumRows.length>0){
    ...
    const colCount=accumRows.length>0?13:12;
